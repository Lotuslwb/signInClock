...
el('#device').onclick = function () {
  console.log(JSON.stringify(ya.device));
};
...
(function () {
  ...
  $stopVoice.onclick = function (e) {
    ...
    ya.recorder.stopVoice(recordLocalId, function (localId) {
  ...
  el('#uploadVoice').onclick = function (e) {
    ya.recorder.uploadVoice(recordLocalId, function (res) {
      ...
      // 成功
      if (obj.ret == 0) {
        console.log('成功：' + JSON.stringify(obj.data));
      } else {
        console.log('失败：' + JSON.stringify(obj));
      }
    });
  };
